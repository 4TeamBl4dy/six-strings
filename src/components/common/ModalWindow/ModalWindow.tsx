...
export const ModalWindow = ({ guitar }: ModalWindowProps) => {
  ...
  return (
    <div>
      <button className="more" onClick={() => setModalIsOpen(true)}>
        Подробнее»
      </button>
      <Modal open={modalIsOpen} onClose={() => setModalIsOpen(false)}>
        ...
        >
          <button className="cross" onClick={() => setModalIsOpen(false)}>
            <CloseIcon sx={{ cursor: '' }} />
          </button>
          <img className="ModalImg" src={guitar.img} alt={guitar.name} />
          <Box className="ModalInfo">
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              ...
              <Typography sx={{ mt: 0.2 }}>{translatedType}</Typography>
            </Box>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Typography fontSize={'17px'} fontWeight={600}>
                Описание:
