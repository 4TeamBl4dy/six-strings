...
router.post('/basket/delete', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await users.updateOne({ _id: new ObjectId(userId) }, { $pull: { basket: { guitarId: guitarId } } });
...
router.patch('/basket/delete', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        for (const item of basket) {
            ...
            const result = await guitars
                .updateOne({ _id: new ObjectId(guitarId) }, { $inc: { amount: -guitarCount } })
                .catch((error) => console.error(error));
        }

        await users.updateOne({ _id: new ObjectId(userId) }, { $set: { basket: [] } });
...
router.post('/basket/confirm', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        for (const item of basket) {
            ...
            await guitars.updateOne({ _id: new ObjectId(guitarId) }, { $set: { amount: newAmount } });
        }
...
router.post('/pay', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const paymentIntent = await stripe.paymentIntents.create({
            ...
            confirm: true,
            return_url:
                process.env.NODE_ENV === 'production' ? 'https://six-strings.makkenzo.com' : 'http://localhost:3000', // URL для возврата
        });
